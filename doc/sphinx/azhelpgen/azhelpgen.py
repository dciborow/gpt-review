...
from gpt_review._gpt_cli import GPTCLI
...
class AzHelpGenDirective(Directive):
    def make_rst(self):
        ...
        for help_file in help_files:
            ...
            yield ".. cli{}:: {}".format(
                "command" if is_command else "group", help_file.command if help_file.command else CLI
            ...
            if help_file.deprecate_info:
                yield "{}:deprecated: {}".format(
                    INDENT, help_file.deprecate_info._get_message(help_file.deprecate_info)
                )
            if not is_command:
                top_group_name = help_file.command.split()[0] if help_file.command else CLI
                yield "{}:docsource: {}".format(
                    INDENT, doc_source_map[top_group_name] if top_group_name in doc_source_map else ""
                )
            else:
            ...
            if is_command and help_file.parameters:
                ...
                for arg in sorted(
                    ...
                    key=lambda p: group_registry.get_group_priority(p.group_name) + str(not p.required) + p.name,
                ):
                    ...
                    if arg.deprecate_info:
                        yield "{}:deprecated: {}".format(
                            DOUBLEINDENT, arg.deprecate_info._get_message(arg.deprecate_info)
                        )
...
def _is_group(parser):
    return getattr(parser, "_subparsers", None) is not None or getattr(parser, "choices", None) is not None
