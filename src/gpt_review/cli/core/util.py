...
SSLERROR_TEMPLATE = (
    "Certificate verification failed. This typically happens when using Azure CLI behind a proxy "
...
QUERY_REFERENCE = "To learn more about --query, please visit: " "'https://docs.microsoft.com/cli/azure/query-azure-cli'"
...
def is_azure_connection_error(error_msg):
    ...
    if "connection error" in error_msg or "connection broken" in error_msg or "connection aborted" in error_msg:
        return True
    return False
...
def shell_safe_json_parse(json_or_dict_string, preserve_order=False, strict=True):
    ...
    try:
        ...
        try:
            ...
            recommendation = (
                "The provided JSON string may have been parsed by the shell. See "
...
def find_child_collection(parent, *args, **kwargs):
    ...
    if arg_len != key_len and path_len != arg_len + 1:
        raise CLIError(
            "command authoring error: args and key_path must have equal number of components, and "
...
def get_az_rest_user_agent():
    ...
    agents = ["python/{}".format(platform.python_version()), "({})".format(platform.platform()), get_az_user_agent()]
