...
from knack.help import (
    ...
    HelpExample as KnackHelpExample,
    HelpParameter as KnackHelpParameter,
    _print_indent,
    CLIHelp,
...
PRIVACY_STATEMENT = """
Welcome to GPT-Review CLI!
---------------------
Use `gpt -h` to see available commands.

"""

WELCOME_MESSAGE = r"""
   ____ ____ _____     ____            _
  / ___|  _ \_   _|   |  _ \ _____   _(_) _____      __
 | |  _| |_) || |_____| |_) / _ \ \ / / |/ _ \ \ /\ / /
 | |_| |  __/ | |_____|  _ <  __/\ V /| |  __/\ V  V /
  \____|_|    |_|     |_| \_\___| \_/ |_|\___| \_/\_/


Use `gpt --version` to display the current version.
...
class CLIPrintMixin(CLIHelp):
    def _print_header(self, cli_name, help_file):
        ...
        if links:
            link_text = (
                "{} and {}".format(", ".join([link["url"] for link in links[0:-1]]), links[-1]["url"])
                if len(links) > 1
                else links[0]["url"]
            )
    ...
    def _get_choices_defaults_sources_str(p):
        choice_str = "  Allowed values: {}.".format(", ".join(sorted([str(x) for x in p.choices]))) if p.choices else ""
        default_value_source = p.default_value_source if p.default_value_source else "Default"
        default_str = (
            "  {}: {}.".format(default_value_source, p.default) if p.default and p.default != argparse.SUPPRESS else ""
        )
...
class AzCliHelp(CLIPrintMixin, CLIHelp):
    ...
    def show_help(self, cli_name, nouns, parser, is_group):
        ...
        help_file = (
            self.command_help_cls(self, delimiters, parser)
            if not is_group
    ...
    def get_examples(self, command, parser, is_group):
        ...
        for example in help_file.examples:
            if example.command and example.name:
                examples.append({"command": strip_command(example.command), "description": example.name})
    ...
    def _register_help_loaders(self):
        ...
        def is_loader_cls(cls):
            return (
                inspect.isclass(cls)
                and cls.__name__ != "BaseHelpLoader"
                and issubclass(cls, help_loaders.BaseHelpLoader)
        ...
        if len(versioned_loaders) != len({ldr.version for ldr in versioned_loaders.values()}):
            ldrs_str = " ".join(
                "{}-version:{}".format(cls_name, ldr.version) for cls_name, ldr in versioned_loaders.items()
            )  # pylint: disable=line-too-long
...
class HelpParameter(KnackHelpParameter):  # pylint: disable=too-many-instance-attributes
    ...
    def update_from_data(self, data):
        ...
        if self.value_sources:
            self.value_sources = [
                str_or_dict if isinstance(str_or_dict, dict) else {"link": {"command": str_or_dict}}
