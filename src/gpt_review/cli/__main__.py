...
try:
    ...
    try:
        ...
        if (
            exit_code == 0
            and az_cli.config.getboolean("auto-upgrade", "enable", False)
            and sys.argv[1] != "upgrade"
            and (sys.argv[1] != "extension" or sys.argv[2] != "update")
        ):
            from azure.cli.core._session import VERSIONS  # pylint: disable=ungrouped-imports
            from azure.cli.core.util import (
                get_cached_latest_versions,
                _VERSION_UPDATE_TIME,
                ...
                if parse(VERSIONS["versions"]["core"]["local"]) < parse(
                    ...
                    if prompt:
                        ...
                        try:
                            ...
                            az_upgrade_run = False
                            err_msg = (
                                "Unable to prompt for auto upgrade as no tty available. "
