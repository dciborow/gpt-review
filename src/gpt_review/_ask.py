...
def validate_parameter_range(namespace):
    ...
    if namespace.max_tokens is not None and (
        namespace.max_tokens < 1 or namespace.max_tokens > 4000
    ):
        raise CLIError("--max_tokens must be an integer between 1 and 4000")
    if namespace.temperature is not None and (
        namespace.temperature < 0 or namespace.temperature > 1
    ):
        raise CLIError("--temperature must be a float between 0 and 1")
    if namespace.top_p is not None and (
        namespace.top_p < 0 or namespace.top_p > 1
    ):
        raise CLIError("--top-p must be a float between 0 and 1")
    if namespace.frequency_penalty is not None and (
        namespace.frequency_penalty < 0 or namespace.frequency_penalty > 2
    ):
        raise CLIError("--frequency-penalty must be a float between 0 and 2")
    if namespace.presence_penalty is not None and (
        namespace.presence_penalty < 0 or namespace.presence_penalty > 2
    ):
